 ...
 * @param {boolean}  [opts.expand=false]     - Expand categories with main colour.
...
function createPalette({
    ...
    function makeCategories() {
        ...
        if (seeds.length === 0) {
            anchors = [main, main];
        } else if (seeds.length === 1 && !expand) {
            anchors = [seeds[0], seeds[0]];
        } else if (seeds.length === 1 && expand) {
            anchors = [seeds[0], main];
        } else if (!expand) {
            anchors = [...seeds];
        } else {
            anchors = [...seeds, main];
...
function downloadPaletteJson(allVariants, filename = 'gruvbox.json') {
